...
public abstract class ShaderProgram {
	...
	public ShaderProgram (String vertexFile, String tessellationControlFile, String tessellationEvaluationFile, String fragmentFile) {
		int vertexShaderID = loadShader(vertexFile, GL20.GL_VERTEX_SHADER);
		int tessellationControlShaderID = loadShader(tessellationControlFile, GL45.GL_TESS_CONTROL_SHADER);
		int tessellationEvaluationShaderID = loadShader(tessellationEvaluationFile, GL45.GL_TESS_EVALUATION_SHADER);
		int fragmentShaderID = loadShader(fragmentFile, GL20.GL_FRAGMENT_SHADER);
	...
	public ShaderProgram (String vertexFile, String tessellationControlFile, String tessellationEvaluationFile, String geometryFile, String fragmentFile) {
		int vertexShaderID = loadShader(vertexFile, GL20.GL_VERTEX_SHADER);
		int tessellationControlShaderID = loadShader(tessellationControlFile, GL45.GL_TESS_CONTROL_SHADER);
		int tessellationEvaluationShaderID = loadShader(tessellationEvaluationFile, GL45.GL_TESS_EVALUATION_SHADER);
		int geometryShaderID = loadShader(geometryFile, GL32.GL_GEOMETRY_SHADER);
	...
	protected int getUniformLocation(String uniformName) {
		...
		if(loc == GL11.GL_INVALID_VALUE) {
			System.err.println("Uniform variable \"" + uniformName + "\" has an invalid value, can't be created by OpenGL.");
		} else if(loc == GL11.GL_INVALID_OPERATION) {
			System.err.println("The program entered is not a valid, usable program, or it has not been linked successfully.");
		} else if(loc == -1) {
	...
	protected void getUniformBufferLocation(String uniformBufferName, int blockBinding) {
		...
		if(uniformIndex == GL11.GL_INVALID_VALUE) {
			System.err.println("Uniform buffer object \"" + uniformBufferName + "\" has an invalid value, can't be created by OpenGL.");
